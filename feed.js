    ...
    fetch(url,params).then((response)=>{
       return response.json();
         ...
         if(!data.status){
            ...
            result =result+ '<div class=" my-16 bg-white rounded-lg mx-16 mr-16 pt-6 shadow-2xl outline-1 outline-double outline-gray-200"><div class="flex ml-4 "><div><img src="./Resources/avatar.png" class="rounded-t-full w-9 h-9"></div><h2 class="ml-3 text-base font-semibold mt-3">'+memeFile[i]['username']+'</h2></div><p class=" mb-3 mt-6 text-gray-700 ml-2 text-xs">'+memeFile[i]['caption']+'</p><img src="'+memeFile[i]['image']+'" class="h-[30rem] w-full bg-cover bg-no-repeat "><div class="flex flex-row"> <p class="ml-3 text-center pt-4 text-sm text-gray-600 font-semibold" id="date">'+ s+'</p> </div></div>';
